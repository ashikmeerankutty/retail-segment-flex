...
const SEGMENT_ANONYMOUS_ID = "SEGMENT_ANONYMOUS_ID";
...
export const sessionDataHandler = {
    ...
    fetchAndStoreNewSession: async ({ formData }: { formData: Record<string, unknown> }) => {
        ...
        try {
            const segmentAnonymousId = localStorage.getItem(SEGMENT_ANONYMOUS_ID);
            const patientInfo = localStorage.getItem(PATIENT_INFO);
            let requestObject;
            if (patientInfo) {
                const patientObject = JSON.parse(patientInfo);
                const patientName = `${patientObject.firstName} ${patientObject.lastName}`;
                requestObject = { formData, segmentAnonymousId, patientName, hasPatient: 1 };
            } else {
                requestObject = { formData, segmentAnonymousId, hasPatient: 0 };
            }
            newTokenData = await contactBackend<Token>("/webchat/initWebchat", requestObject);
        } catch (e) {
    ...
    clear: () => {
        ...
        localStorage.removeItem(SEGMENT_ANONYMOUS_ID)
    }
};
